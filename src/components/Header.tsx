              ...
              >
                {item.name === 'Products' ? (
                  ...
                  >
                    ...
                    >
                      {item.name}
                      <span className={`absolute -bottom-1 left-0 w-0 h-0.5 bg-secondary transition-all duration-300 group-hover:w-full ${isActive(item.href) ? 'w-full' : ''}`}></span>
                    </Link>
                        ...
                        <div className="p-2">
                          {productCategories.map((category) => (
                            <div
                              key={category.id}
                              className="relative"
                              onMouseEnter={() => category.subProducts.length > 0 ? handleCategoryHover(category.id) : null}
                            >
                              <Link
                                to={`/products/${category.id}`}
                                className={`flex items-center justify-between w-full px-3 py-2 text-sm text-muted-foreground hover:text-secondary hover:bg-muted/50 rounded-md transition-colors ${category.subProducts.length > 0 ? '' : 'justify-start'}`}
                              >
                                <span>{category.title}</span>
                                {category.subProducts.length > 0 && <ChevronRight className="w-4 h-4" />}
                              </Link>

                              {/* Second Level Dropdown - Only for categories with subProducts */}
                              {activeCategory === category.id && category.subProducts.length > 0 && (
                                <motion.div
                                  ...
                                  <div className="p-2">
                                    {category.subProducts.map((product) => (
                                      ...
                                      >
                                        ...
                                        <div className="text-xs text-muted-foreground/70">{product.description}</div>
                                      </Link>
                                    ))}
                                  </div>
                                </motion.div>
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side="right" className="w-80 sm:w-96">
                ...
                {navigationItems.map((item) => (
                  item.name === 'Products' ? (
                    <Collapsible key={item.name} open={mobileProductsOpen} onOpenChange={setMobileProductsOpen}>
                      <CollapsibleTrigger className={`flex items-center justify-between w-full text-lg font-medium transition-colors hover:text-secondary p-2 rounded-lg hover:bg-muted/50 ${isActive(item.href) ? 'text-secondary bg-muted/30' : 'text-muted-foreground'}`}>
                        <Link to={item.href} onClick={() => setIsOpen(false)} className="flex-1 text-left">
                          {item.name}
                        </Link>
                    ...
                    <Link
                      ...
                      to={item.href}
                      onClick={() => setIsOpen(false)}
                      className={`text-lg font-medium transition-colors hover:text-secondary p-2 rounded-lg hover:bg-muted/50 ${isActive(item.href) ? 'text-secondary bg-muted/30' : 'text-muted-foreground'
                        }`}
                    >
                      {item.name}
                    </Link>
